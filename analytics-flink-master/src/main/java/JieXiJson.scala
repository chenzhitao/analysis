  ...
  def main(args: Array[String]): Unit = {
    ...
    val DateStream = stream.map(line => {
      ...
      ceshi(
        ...
        yhq_denomination,order_id,product_id,payment,is_have_material,duration,page_id,element_id,second_commodity
      )
 ...
 class tomysql() extends RichSinkFunction[ceshi]{
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    insertStmt = conn.prepareStatement("INSERT INTO maidian (distinct_id,track_id,time1,app_version,lib,lib_version,manufacturer," +
      ...
      "product_id,payment,is_have_material,duration,second_commodity) VALUE(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
      "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
      "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
      "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," +
      "?,?,?,?,?,?,?,?,?,?)")
  ...
  override def invoke(value: ceshi, context: SinkFunction.Context[_]): Unit = {
    ...
    insertStmt.setString(90,value.second_commodity)
...
class HbaseSink() extends RichSinkFunction[ceshi]{
  ...
  override def invoke(value: ceshi, context: SinkFunction.Context[_]): Unit = {
    ...
    put.add(Bytes.toBytes("f1"),Bytes.toBytes("second_commodity"),Bytes.toBytes(value.second_commodity.toString))
