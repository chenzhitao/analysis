...
import com.elai.common.core.domain.entity.SysRole;
...
import com.elai.system.service.ISysPostService;
import com.elai.system.service.ISysRoleService;
...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user)
    {
        ...
        if(StringUtils.isNotEmpty(user.getLesseeId())){
            BaseLesseeInfo lesseeInfo= baseLesseeInfoService.selectBaseLesseeInfoByLesseeId(user.getLesseeId());
            if(lesseeInfo==null){
                return AjaxResult.error("新增用户'" + user.getUserName() + "'失败，租户ID不存在");
            }
            if("02".equals(lesseeInfo.getStatus())){
                return AjaxResult.error("新增用户'" + user.getUserName() + "'失败，租户异常请联系管理员");
            }
            if(lesseeInfo.getBeginDate().getTime()> DateUtils.getNowDate().getTime() || DateUtils.getNowDate().getTime()>lesseeInfo.getEndDate().getTime()){
                return AjaxResult.error("新增用户'" + user.getUserName() + "'失败，租户不在有效期内，请联系管理员");
            }
        }
    ...
    public AjaxResult edit(@Validated @RequestBody SysUser user)
    {
        ...
        if(StringUtils.isNotEmpty(user.getLesseeId())){
            BaseLesseeInfo lesseeInfo= baseLesseeInfoService.selectBaseLesseeInfoByLesseeId(user.getLesseeId());
            if(lesseeInfo==null){
                return AjaxResult.error("修改用户'" + user.getUserName() + "'失败，租户ID不存在");
            }
            if("02".equals(lesseeInfo.getStatus())){
                return AjaxResult.error("修改用户'" + user.getUserName() + "'失败，租户异常请联系管理员");
            }
            if(lesseeInfo.getBeginDate().getTime()> DateUtils.getNowDate().getTime() || DateUtils.getNowDate().getTime()>lesseeInfo.getEndDate().getTime()){
                return AjaxResult.error("修改用户'" + user.getUserName() + "'失败，租户不在有效期内，请联系管理员");
            }
        }
